...
public class MovieTimeSlotEditor implements Writable{
	...
	public void create() {
		...
		List<MovieTimeSlot> showtimeData = SerializeDB.getMovieTimeSlotList("src/data/cinema_showtime.dat");
		...
		if (movieNames.size() == 0) {
			...
			MovieTimeSlot st = new MovieTimeSlot(showMovie, showCineplex, showCinema, showDate, showTime);
			showtimeData.add(st);				
		}
		
		SerializeDB.writeToMovieTimeSlotList(showtimeData);
	...
	@SuppressWarnings("resource")
	public List<MovieTimeSlot> updateLocation(int index) {
		Scanner sc = new Scanner(System.in);
		List<MovieTimeSlot> showtimeData = SerializeDB.getMovieTimeSlotList("src/data/cinema_showtime.dat");
		List<Cineplex> cineplexData = SerializeDB.getCineplexList("src/data/cineplex.dat");
		List<String> cineplexNames = new ArrayList<String>();
	...
	@SuppressWarnings("resource")
	public List<MovieTimeSlot> updateTimeslot(int index) {
		List<MovieTimeSlot> showtimeData = SerializeDB.getMovieTimeSlotList("src/data/cinema_showtime.dat");
		boolean toggle_flag1 = true, toggle_flag2 = true;
